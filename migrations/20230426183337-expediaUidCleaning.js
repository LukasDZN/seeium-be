...
export const up = async (db) => {
  ...
  while (await cursor.hasNext()) {
    ...
    const uniqueHotelNameIdentifier =
      document.providers.expedia.uniqueHotelNameIdentifier

    // Check if uniqueHotelNameIdentifier exists and is not null
    if (uniqueHotelNameIdentifier) {
      counter++
      const updatedUniqueHotelNameIdentifier = removeExtraFieldsInExpediaUrl(
        uniqueHotelNameIdentifier
      )
